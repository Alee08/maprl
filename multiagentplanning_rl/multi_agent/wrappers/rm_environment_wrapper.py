...
class RMEnvironmentWrapper:
    ...
    def step(self, actions):
        ...
        for agent in self.agents:
            if not self.env.active_agents[agent.name] and not self.env.centralized:
                continue
            rm = agent.get_reward_machine()
            current_state = infos[agent.name]["prev_s"]
            next_state_ = infos[agent.name][
                "s"
    ...
    def _get_qrm_experiences(
        ...
        for state_rm in all_states:
            event = rm.event_detector.detect_event(next_state, state_rm)
            (
